...
class YoloFollower(Node):
    def __init__(self):
        ...
        self.detection_pub = self.create_publisher(Bool, '/obstacle_detected', 10)
    ...
    def detect_and_navigate(self):
        ...
        for det in result.boxes:
            ...
            found_car = True  # ✅ 탐지됨 표시

            # ↓ 기존 속도 계산, pt 변환, TF 등 그대로 유지

            break  # 첫 차량만 추적

        # ✅ 탐지 상태 업데이트 및 퍼블리시
        if found_car and not self.detection_state:
            self.detection_pub.publish(Bool(data=True))
            self.detection_state = True
        elif not found_car and self.detection_state:
